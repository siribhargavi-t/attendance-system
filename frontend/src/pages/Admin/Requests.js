    ...
    return (
        <div className="space-y-4">
            ...
            {requests.length === 0 ? (
                ...
                requests.map(req => (
                    <div key={req._id} className={`rounded-2xl p-5 border transition-all animate-fade-in-up
        ...
        <div className="space-y-4">
            ...
            {loading ? (
                ...
                leaves.map(leave => {
                    ...
                    return (
                        <div key={leave._id} className={`rounded-2xl p-5 border transition-all animate-fade-in-up
                            ${isDark ? 'bg-slate-800/70 border-slate-700/50' : 'bg-white border-slate-100 shadow-sm hover:shadow-md'}`}>
                    ...
                    <div className={`w-full max-w-md rounded-2xl overflow-hidden shadow-2xl`}
                        ...
                        <div className={`px-6 py-4 border-b ${isDark ? 'border-slate-700' : 'border-slate-100'} flex items-center gap-3`}>
                            ...
                            <div>
                                ...
                                <p className={`text-xs ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Add a comment for the student (optional)</p>
                            </div>
                            <button onClick={() => setCommentModal(null)} className={`ml-auto ${isDark ? 'text-slate-600 hover:text-slate-300' : 'text-slate-400 hover:text-slate-600'}`}><X className="w-4 h-4" /></button>
                        </div>
                        <div className="p-6 space-y-4">
                            <div className="flex items-center gap-2">
                                <MessageCircle className={`w-3.5 h-3.5 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} />
                                <span className={`text-xs font-semibold uppercase tracking-wider ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Admin Comment (Optional)</span>
                            </div>
                            <textarea rows={3} value={adminComment} onChange={e => setAdminComment(e.target.value)}
                                ...
                                className={`w-full px-4 py-3 text-sm rounded-xl outline-none border resize-none transition-all
                                    ${isDark
                                        ? 'bg-slate-900 text-white placeholder-slate-600 border-slate-700 focus:border-indigo-500'
                                        : 'bg-slate-50 text-slate-800 placeholder-slate-400 border-slate-200 focus:border-indigo-400'}`} />
...
const Requests = () => {
    ...
    return (
        <div className="space-y-6 animate-fade-in-up">
            {/* Tab bar */}
            <div className={`flex gap-1 p-1.5 rounded-2xl ${isDark ? 'bg-slate-800/80 border border-slate-700' : 'bg-slate-100 border border-slate-200'}`}>
                {tabs.map(tab => (
